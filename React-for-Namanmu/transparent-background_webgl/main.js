...
const transparentGreenCanvas = document.querySelector(
  "canvas#transparent_green_canvas"
);
...
const callBtnTransparentCanvas = document.querySelector(
  "button#call_transparent_green"
);
const callBtnTransparentWebgl = document.querySelector(
  "button#call_transparent_webgl"
);
...
const greenTransparencyEnabled = document.querySelector(
  "input#show_green_transparency"
);
...
async function sendVideo(stream) {
  ...
  document
    .querySelectorAll(".senderControl")
...
async function start() {
  ...
  videoElement.onplaying = async () => {
    ...
    segmentedCanvas = greenScreenEnabled.checked
      ? greenScreenCanvas
      : offscreenCanvas;
  ...
  callBtnGreen.onclick = () =>
    sendVideo(greenScreenCanvas.captureStream(FRAME_RATE));
  callBtnTransparent.onclick = () =>
    sendVideo(transparentGreenCanvas.captureStream(FRAME_RATE));
  callBtnTransparentCanvas.onclick = () =>
    sendVideo(transparentGreenCanvas.captureStream(FRAME_RATE));
  callBtnTransparentWebgl.onclick = () =>
    sendVideo(webglCanvas.captureStream(FRAME_RATE));
}
...
transparencyEnabled.onclick = async () => {
  ...
  transparentCanvas.parentElement.hidden =
    !transparentCanvas.parentElement.hidden;
  await videoElement.play();
};
...
greenScreenEnabled.onclick = async () => {
  ...
  greenScreenCanvas.parentElement.hidden =
    !greenScreenCanvas.parentElement.hidden;
  if (greenScreenCanvas.parentElement.hidden) await videoElement.play();
};

greenTransparencyEnabled.onclick = async () => {
  console.log("Changing transparent canvas display state");
  transparentGreenCanvas.parentElement.hidden =
    !transparentGreenCanvas.parentElement.hidden;
  await videoElement.play();
};
